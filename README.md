...
it is not, because the id was in the answer that went missing. A failure Pingen
*did* answer — a 404, a `conflict_state` — is left as it is: it already says what
happened, and a warning on every one of those is noise.
...
| `fetch failed` / `Zeitüberschreitung` on send or submit | The request left, the answer did not — Pingen may have taken the letter anyway | Do **not** repeat the call. Check first: `pingen_get_letter` for a submit, `pingen_list_letters` (newest first) for an `auto_send: true` create. Only then decide. |
...
letter that stayed put — while a failure Pingen did answer is left alone — that
